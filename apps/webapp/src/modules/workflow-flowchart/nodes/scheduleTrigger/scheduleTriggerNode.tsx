...
import type { FlowchartRfNode, NodeDefinition, ScheduleTriggerConfiguration } from "../types"
import { renameNodeAction } from "../nodeActions"

function ScheduleTriggerFlowNode({ data }: NodeProps) {
...
export const scheduleTriggerNode: NodeDefinition<ScheduleTriggerConfiguration> = {
  type: "node:scheduletrigger",
  defaultTitle: "Schedule Trigger",
  description: "Dispara el flujo según una expresión cron.",
  Icon: Clock,
  category: "triggers",
  ...
  FlowComponent: ScheduleTriggerFlowNode,
  actions: [renameNodeAction],
  handlers: {
    onConnect(event) {
      if (event.targetNode.type === "node:scheduletrigger") return false
      return true
    },
  },
}
